...
public interface MunicipalityRepository extends R2dbcRepository<Municipality, UUID> {
     ...
     * El schema se define en @Table de Municipality, no aquí.
     */
    @Query("SELECT * FROM municipalities WHERE LOWER(name) = LOWER(:name) AND is_active = true")
    Mono<Municipality> findByNameIgnoreCase(@Param("name") String name);

    /**
     * Buscar municipio por nombre normalizado.
     */
    @Query("SELECT * FROM municipalities WHERE normalized_name = LOWER(:normalizedName) AND is_active = true")
    Mono<Municipality> findByNormalizedName(@Param("normalizedName") String normalizedName);

    /**
     * Buscar municipios por provincia.
     */
    @Query("SELECT * FROM municipalities WHERE LOWER(province) = LOWER(:province) AND is_active = true")
     ...
     */
    @Query("SELECT * FROM municipalities WHERE is_active = true ORDER BY name")
    Flux<Municipality> findAllActive();

    /**
     * Verificar si un municipio existe por coordenadas (aproximadas).
     */
    @Query("SELECT COUNT(*) > 0 FROM municipalities " +
